...
export default function HistoryEntry({timestamp, player1, player2,username}: HistoryEntryProps) {
  const won = ((player1.username === username && player1.victory) || (player2.username === username && player2.victory));
  const date = new Date(timestamp);
  return (
      <li className={`flex flex-col px-2 bg-slate-500 border-b-2 last:border-b-0 border-slate-700 ${won ? "bg-green-900" : "bg-red-900"}`}>
